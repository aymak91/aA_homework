class Board
  ...
  def initialize(name1, name2)
    ...
    place_stones #calls place stones to add 4 stones to non-store cups    
  end

  def place_stones
    @cups.each_with_index do |cup, idx|
      next if idx == 6 || idx == 13 #0..5 and 7..12 are non-store cups
      4.times do # next evaluates next iterations without further evaluation
        cup << :stone
      end
    end
  end
